...
public class ResetCredentialChooseUser implements Authenticator, AuthenticatorFactory {
    ...
    public void action(AuthenticationFlowContext context) {
        ...
        String method = formData.getFirst(ResetCredentialMethod.METHOD);
        ...
        switch (method) {
            ...
            case ResetCredentialMethod.MAGIC_LINK: {
                if (!username.contains("@")) {
                    Response challenge = context.form()
                            .addError(new FormMessage(Validation.FIELD_USERNAME, "emailInvalid"))
                            .createPasswordReset();
                    context.failureChallenge(AuthenticationFlowError.INVALID_USER, challenge);
                    return;
                }
            ...
            case ResetCredentialMethod.PHONE_OTP: {
                if(!Utils.isPhoneNumberValid(username)) {
                    Response challenge = context.form()
                            .addError(new FormMessage(Validation.FIELD_USERNAME, "phoneNumberInvalid"))
                            .createPasswordReset();
                    context.failureChallenge(AuthenticationFlowError.INVALID_USER, challenge);
                    return;
                }
            ...
            default:
                event.clone().error("invalidResetCredentialMethod");
                Response challenge = context.form()
                        .addError(new FormMessage(ResetCredentialMethod.METHOD, "invalidResetCredentialMethod"))
                        .createPasswordReset();
...
//        context.getAuthenticationSession().setAuthNote(AbstractUsernameFormAuthenticator.ATTEMPTED_USERNAME, username);
        if (user == null) {
            ...
            // status false: user không tồn tại hoặc có lỗi xảy ra khi đồng bộ.
            if (!status) {
                event.clone()
        ...
        context.getAuthenticationSession().setAuthNote(ResetCredentialMethod.TARGET_USERNAME_INFO, username);
        context.getAuthenticationSession().setAuthNote(ResetCredentialMethod.METHOD, method);
        context.success();
    }
    ...
    private boolean handleSyncV3(AuthenticationFlowContext context, RealmModel realm, String username) {
        ...
        if (!clientId.equals(SyncV3Service.VBEE_TTS_CLIENT))
            return false;
        UserSyncRequest userSyncRequest = UserSyncRequest.build().realmId(realm.getName()).clientId(clientId).username(username);
