...
public class ResetCredentialMultiMethods implements Authenticator, AuthenticatorFactory {
    private static final Logger logger = Logger.getLogger(ResetCredentialMultiMethods.class);
    ...
    public void authenticate(AuthenticationFlowContext context) {
        ...
        String method = context.getAuthenticationSession().getAuthNote(ResetCredentialMethod.METHOD);
        switch (method) {
            case ResetCredentialMethod.PHONE_OTP:
            case ResetCredentialMethod.EMAIL_OTP:
                authenticateWithOTP(context);
                break;
            case ResetCredentialMethod.MAGIC_LINK:
    ...
    private void sendOtpCodeThroughMobilePhone(AuthenticationFlowContext context, String code, int ttl) throws IOException {
        ...
        //VoiceOtpServiceFactory.get(config.getConfig()).send(urlCallCenter, accessToken, mobileNumber, code);
    ...
    private void challengeOtpForm(AuthenticationFlowContext context) {
        AuthenticationSessionModel authSession = context.getAuthenticationSession();
        String resetCredentialMethod = authSession.getAuthNote(ResetCredentialMethod.METHOD);
        String targetUsername = authSession.getAuthNote(ResetCredentialMethod.TARGET_USERNAME_INFO);
        String oldResendDuration = authSession.getAuthNote(ResetCredentialMethod.RESEND_DURATION);

        try {
            OtpModel otpModel = new OtpModel(resetCredentialMethod, targetUsername, oldResendDuration);
            context.challenge(context.form().setAttribute(ResetCredentialMethod.OTP_MODEL, otpModel).createForm(OTP_CODE_TPL));
        } catch (Exception e) {
    ...
    private void authenticateWithOTP(AuthenticationFlowContext context) {
        ...
        authSession.setAuthNote(ResetCredentialMethod.CODE, code);
        authSession.setAuthNote(ResetCredentialMethod.TTL, Long.toString(System.currentTimeMillis() + (ttl * 1000L)));
        authSession.setAuthNote(ResetCredentialMethod.RESEND_DURATION, Long.toString(System.currentTimeMillis() + (resendDuration * 1000L)));

        try {
            String resetCredentialMethod = authSession.getAuthNote(ResetCredentialMethod.METHOD);
            if(resetCredentialMethod.equals(ResetCredentialMethod.EMAIL_OTP))
    ...
    private void actionWithOTP(AuthenticationFlowContext context) {
        ...
        AuthenticationSessionModel authSession = context.getAuthenticationSession();
        String code = authSession.getAuthNote(ResetCredentialMethod.CODE);
        String ttl = authSession.getAuthNote(ResetCredentialMethod.TTL);
        String resetCredentialMethod = authSession.getAuthNote(ResetCredentialMethod.METHOD);
        String targetUsername = authSession.getAuthNote(ResetCredentialMethod.TARGET_USERNAME_INFO);
        String oldResendDuration = authSession.getAuthNote(ResetCredentialMethod.RESEND_DURATION);
        ...
        if (isValid) {
            ...
            if (execution.isRequired()) {
                ...
                context.failureChallenge(AuthenticationFlowError.INVALID_CREDENTIALS,
                        context.form().setAttribute(ResetCredentialMethod.OTP_MODEL, otpModel)
    ...
    public String getDisplayType() {
        return "Reset Credential With OTP or Magic link";
    }
    ...
    public List<ProviderConfigProperty> getConfigProperties() {
        return Arrays.asList(
                ...
                new ProviderConfigProperty(OtpConfig.SIMULATION, "Simulation mode", "In simulation mode, the SMS won't be sent, but printed to the server logs", ProviderConfigProperty.BOOLEAN_TYPE, true)
        );
    }
