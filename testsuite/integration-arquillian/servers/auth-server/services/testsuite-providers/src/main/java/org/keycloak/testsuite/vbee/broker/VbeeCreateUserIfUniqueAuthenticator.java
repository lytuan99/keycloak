 ...
 */
public class VbeeCreateUserIfUniqueAuthenticator extends AbstractIdpAuthenticator {
    private static Logger logger = Logger.getLogger(VbeeCreateUserIfUniqueAuthenticator.class);
    ...
    protected void authenticateImpl(AuthenticationFlowContext context, SerializedBrokeredIdentityContext serializedCtx, BrokeredIdentityContext brokerContext) {
        ...
        String provider = brokerContext.getIdpConfig().getAlias();
        String email = brokerContext.getEmail();
        if (email != null && !context.getRealm().isDuplicateEmailsAllowed() && provider.toLowerCase().equals("facebook")) {
            UserModel user = session.users().getUserByEmail(realm, email);
            if (user != null) {
                FederatedIdentityModel federatedIdentity = session.users().getFederatedIdentity(realm, user, provider);
                if (federatedIdentity != null) {
                    FederatedIdentityModel newFederatedIdentity = new FederatedIdentityModel(federatedIdentity, brokerContext.getId());
                    session.users().removeFederatedIdentity(realm, user, provider);
        ...
        if (duplication == null) {
            ...
            if (config != null && Boolean.parseBoolean(config.getConfig().get(VbeeCreateUserIfUniqueAuthenticatorFactory.REQUIRE_PASSWORD_UPDATE_AFTER_REGISTRATION))) {
            ...
            context.getAuthenticationSession().setAuthNote(BROKER_REGISTERED_NEW_USER, "true");
            handleSyncUserV3(context, brokerContext, realm, username, federatedUser.getId());
            context.success();
        } else {
    ...
    private void handleSyncUserV3(AuthenticationFlowContext context , BrokeredIdentityContext brokerContext, RealmModel realm, String username, String userId) {
        String clientId = context.getAuthenticationSession().getClient().getClientId();
        if(!clientId.equals(SyncV3Service.VBEE_TTS_CLIENT)) return;

        String providerUserId = brokerContext.getId();
        String provider = brokerContext.getIdpConfig().getAlias();

        UserSyncRequest userSyncRequest = UserSyncRequest.build(realm.getName(), clientId).username(username).provider(provider).providerUserId(providerUserId).userId(userId);
